-- Nâng cấp chức năng khách đặt món giao tận nơi theo quy trình chuẩn:
-- xem menu -> giỏ hàng -> checkout -> địa chỉ/phạm vi/phí giao -> thanh toán
-- -> tạo đơn -> nhà hàng xác nhận -> bếp chế biến -> bàn giao -> giao thành công.
-- Đơn hẹn giờ chỉ được chuyển xuống bếp khi đến thời điểm cần bắt đầu chuẩn bị.
...
CREATE TABLE IF NOT EXISTS giao_hang_don_hang (
    ...
    so_nha VARCHAR(50),
    ten_duong VARCHAR(200),
    thong_tin_dia_chi VARCHAR(500),
    ...
    loai_thoi_gian_nhan VARCHAR(20) NOT NULL DEFAULT 'SOM_NHAT',
    thoi_gian_nhan_mong_muon TIMESTAMP,
...
ALTER TABLE giao_hang_don_hang ADD COLUMN IF NOT EXISTS ten_duong VARCHAR(200);
ALTER TABLE giao_hang_don_hang ADD COLUMN IF NOT EXISTS thong_tin_dia_chi VARCHAR(500);
...
ALTER TABLE giao_hang_don_hang ADD COLUMN IF NOT EXISTS loai_thoi_gian_nhan VARCHAR(20) NOT NULL DEFAULT 'SOM_NHAT';
ALTER TABLE giao_hang_don_hang ADD COLUMN IF NOT EXISTS thoi_gian_nhan_mong_muon TIMESTAMP;
...
ALTER TABLE giao_hang_don_hang ALTER COLUMN trang_thai_giao_hang SET DEFAULT 'CHO_XAC_NHAN';
